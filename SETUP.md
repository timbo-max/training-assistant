# Training Assistant — Beta Setup Guide

Hi! I'm going to help you set up your personal AI training assistant step by step. This is a system that automatically syncs your Garmin, Hevy, and TrainingPeaks data and lets you chat with an AI coach via Telegram.

Before we start, here's what you'll need:
- A Garmin account with a Garmin device (Fenix, Forerunner, etc.)
- A Hevy PRO account ($3 USD/month) — needed for gym tracking
- A TrainingPeaks account (free or paid) — optional, for planned sessions
- A Telegram account
- About 45-60 minutes

We'll set up the following services together — all free unless noted:
- GitHub (free) — stores the code
- Railway (free) — runs the backend
- Supabase (free) — stores your data
- Anthropic API (~$1-2/month) — powers the AI
- Telegram bot (free) — your chat interface
- cron-job.org (free) — schedules daily syncs

---

## Let's get started

Please paste the following prompt into a new Claude chat window and Claude will guide you through each step interactively:

---

> I want to set up a personal AI training assistant that syncs my Garmin, Hevy, and TrainingPeaks data and lets me chat with an AI coach via Telegram. Please guide me through the setup step by step, one step at a time. Wait for me to confirm each step is complete before moving to the next. Here is the full setup guide:
>
> WHAT WE ARE BUILDING:
> A Python backend hosted on Railway that pulls data from Garmin Connect, Hevy, and TrainingPeaks into a Supabase database. A Telegram bot powered by Claude AI lets you chat with your training data. A daily cron job syncs your data automatically at 6am.
>
> STEP 1 — GITHUB
...
> Job 1 — Daily sync: URL is https://YOUR_RAILWAY_URL/sync?token=YOUR_SYNC_SECRET, schedule is every day at 6:00 AM in your local timezone.
...
>
> You can also type /help to see available commands, and /sync to manually pull today's data.
>
...
## Telegram commands once set up

Once everything is running, here are the commands you can use in your Telegram bot:

- `/sync` — pulls today's data immediately
- `/sync 2026-04-15` — syncs a specific date
- `/clear` — clears conversation history
- `/help` — shows available commands
- `"suggest a gym session"` — generates a workout and creates it in Hevy when you confirm

## What it costs to run

- Railway, Supabase, Telegram, cron-job.org — all free
- Anthropic API — roughly $1-2/month depending on how much you chat
- Hevy PRO — $3/month (you likely already have this)

## If something goes wrong

Check your Railway deploy logs first — most issues show up there clearly. Common problems:

- Garmin rate limited (429 error) — wait 2-3 hours and try again
- Telegram webhook not working — re-run the webhook URL from Step 8
- Supabase connection error — double check your SUPABASE_URL and SUPABASE_KEY variables in Railway
